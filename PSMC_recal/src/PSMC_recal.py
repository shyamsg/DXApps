...
def main(psmcfa, outname, timemax, parfile):

    # The following line(s) initialize your data object inputs on the platform
    # into dxpy.DXDataObject instances that you can start using immediately.

    psmcfa = dxpy.DXFile(psmcfa)
    parfile = dxpy.DXFile(parfile)

    # The following line(s) download your file inputs to the local file system
    # using variable names for the filenames.

    dxpy.download_dxfile(psmcfa.get_id(), "psmcfa")
    dxpy.download_dxfile(parfile.get_id(), "parfile")

    # Fill in your application code here.
    subprocess.check_call(['psmc', '-t', str(round(timemax,4)), '-i', 'parfile', '-o', outname, 'psmcfa'])
