...
def main(infile, outroot, xchr=True, recalnums=1, skip=20, timemax=7500000.0):

    # The following line(s) initialize your data object inputs on the platform
    # into dxpy.DXDataObject instances that you can start using immediately.

    inf = dxpy.DXFile(infile)

    # The following line(s) download your file inputs to the local file system
    # using variable names for the filenames.

    dxpy.download_dxfile(inf.get_id(), "infile")
    outname2 = outroot + '.psmc'

    # Fill in your application code here.
    #create the psmcfa file
    subprocess.check_call(['psmc', '-t', '15', '-r', '5', '-p', "4+25*2+4+6", '-o', 'test.psmc', 'infile'])
    ...
    while (recalnums > 1):
        ...
        subprocess.check_call(['psmc', '-t', str(round(tmaxNew,4)), '-i', parfile, '-o', 'test.psmc', 'infile'])
        recalnums -= 1
        print 'Recals left', recalnums
        sys.stdout.flush()
    (tmaxNew, parfile) = writeRecalFile('test.psmc', timemax, skip, xchr)
    subprocess.check_call(['psmc', '-t', str(round(tmaxNew,4)), '-i', parfile, '-o', outname2, 'infile'])
